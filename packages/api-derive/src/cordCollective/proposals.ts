...
function _proposalsFrom(
  instanceId: string,
  api: ApiInterfaceRx,
  section: string
): (hashes: (Hash | Uint8Array | string)[]) => Observable<DeriveCollectiveProposal[]> {
  return memo(
    instanceId,
    (hashes: (Hash | Uint8Array | string)[]): Observable<DeriveCollectiveProposal[]> =>
      (isFunction(api.query[section]?.proposals) && hashes.length
        ...
        : of<Result>([[], [], []])
      ).pipe(map((result) => parse(api, result)))
  );
}
...
export function hasProposals(instanceId: string, api: ApiInterfaceRx, _section: Collective): () => Observable<boolean> {
  ...
  return memo(instanceId, (): Observable<boolean> => of(isFunction(api.query[section]?.proposals)));
}
...
export function proposalCount(
  ...
  return memo(
    instanceId,
    (): Observable<u32 | null> =>
      isFunction(api.query[section].proposalCount) ? api.query[section as 'council'].proposalCount() : of(null)
  );
}

export function proposalHashes(
  instanceId: string,
  api: ApiInterfaceRx,
  _section: Collective
): () => Observable<Hash[]> {
  ...
  return memo(
    instanceId,
    (): Observable<Hash[]> =>
      isFunction(api.query[section]?.proposals) ? api.query[section as 'council'].proposals() : of([])
  );
}
...
export function proposals(
  ...
  return memo(instanceId, (): Observable<DeriveCollectiveProposal[]> => getHashes().pipe(switchMap(proposalsFrom)));
}
...
export function proposal(
  ...
  return memo(
    instanceId,
    (hash: Hash | Uint8Array | string): Observable<DeriveCollectiveProposal | null> =>
      isFunction(api.query[section]?.proposals) ? proposalsFrom([hash]).pipe(map(([proposal]) => proposal)) : of(null)
  );
}
